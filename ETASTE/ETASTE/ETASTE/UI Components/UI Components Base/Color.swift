//
//  Color.swift
//  ETASTE
//
//  Created by 石黒晴也 on 2019/07/31.
//  Copyright © 2019 ETASTE. All rights reserved.
//

import UIKit

extension UIColor {
    static var etasteYellow: UIColor {
        return UIColor.hex(string: "FFDB73", alpha: 1.0)
    }
    
    static var etasteOrange: UIColor {
        return UIColor.hex(string: "F7B500", alpha: 1.0)
    }
    
    static var etastePaleYellow: UIColor {
        return UIColor.hex(string: "FBDA7F", alpha: 1.0)
    }
    
    static var etasteDarkerYellow: UIColor {
        return UIColor.hex(string: "FDB730", alpha: 1.0)
    }
    
    static var placeHoldersGray: UIColor {
        return UIColor.hex(string: "EFEFEF", alpha: 1.0)
    }
    
    static var paleWhite: UIColor {
        return UIColor.hex(string: "FFFFFF", alpha: 1.0)
    }
    
    static var darkGray: UIColor {
        return UIColor.hex(string: "979797", alpha: 1.0)
    }
    
    static var etasteGray: UIColor {
        return UIColor.hex(string: "C5C5C5", alpha: 1.0)
    }
    
    static var black: UIColor {
        return UIColor.hex(string: "000000", alpha: 1.0)
    }
    
    static var paleBlack: UIColor {
        return UIColor.hex(string: "5F5F5F", alpha: 1.0)
    }
    
    static var white: UIColor {
        return UIColor.hex(string: "FFFFFF", alpha: 1.0)
    }
    
}
